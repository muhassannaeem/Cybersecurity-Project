...
35. Design a centralized logging architecture using the ELK stack (Elasticsearch, Logstash, Kibana) or an equivalent solution.
36. Configure each microservice container to ship structured logs to the central logging system (e.g., via Filebeat or Fluentd).
37. Define log formats that capture both system events and attacker behavior for post‑incident analysis.
